...
export const PaperCard: React.FC<PaperCardProps> = ({ paper, compact = false }) => {
  ...
  return (
    <motion.div 
      ...
      className="flex flex-col p-5 rounded-2xl border border-brand-border bg-white hover:border-brand-accent hover:shadow-float cursor-pointer transition-all duration-300 group"
      onClick={() => {
    ...
    >
      <div className="flex justify-between items-start gap-4 mb-3">
        ...
        <button 
          ...
          className={`shrink-0 p-2 rounded-full transition-colors ${isSaved ? 'text-brand-accent bg-blue-50' : 'text-brand-textMuted hover:bg-gray-50'}`}
        >
      ...
      <div className="flex items-center gap-3 text-[12px] text-brand-textMuted font-medium mb-3">
        ...
        <span className="px-2 py-0.5 rounded-md bg-gray-100 text-brand-textSoft">{paper.venue}</span>
      </div>
